...
export const regionsCode: { [key: string]: string } = {
...
export class PhoneValidators {
  ...
  public static isPhoneNumber(local: string): ValidatorFn {
    const validator = (control: AbstractControl): ValidationErrors => {
      ...
      const phoneParser: PhoneNumberUtil = PhoneNumberUtil.getInstance();

      let error = { noPhoneNumber: true };
      try {
        const phoneNumber: libPhoneNumber.PhoneNumber = phoneParser.parse(
          control.value,
          local
        );
  ...
  public static isPossibleNumberWithReason(local: string): ValidatorFn {
    const validator = (control: AbstractControl): ValidationErrors => {
      ...
      let error: ValidationErrors = { noPhoneNumber: true };
      try {
        const phoneNumber: libPhoneNumber.PhoneNumber = phoneParser.parse(
          control.value,
          local
        );
        const reason: PhoneNumberUtil.ValidationResult = phoneParser.isPossibleNumberWithReason(
          phoneNumber
        );
        switch (reason) {
